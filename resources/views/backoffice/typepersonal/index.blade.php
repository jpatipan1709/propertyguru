                        ...
                        <div class="col-lg-12 col-md-12">
                            <div class="card">
                                ...
                                <div class="card-block">
                                    ...
                                    <form action="{{ url('typeperson') }}" method="post">
                                            @csrf
                                            <div class="form-row">
                                                ...
                                                placeholder="Type of person" autocomplete="off" value="{{ old('typepersonal_name') }}">
                                                </div>
                                            </div>
                                            <div class="form-row">
                                        ...
                                        <table id="table2" class="table table-bordered table-strip table-hover">
                                            <thead>
                                                <tr>
                                                    ...
                                                    <th class="text-center">Type of person</th>
                                                    <th class="text-center" width="20%">Manage</th>
                                                </tr>
                                            </thead>
                                            <tbody>
                                                @foreach ($typepersonals as $key => $typepersonal)
                                                <tr class="text-center">
                                                    <td>{{ ++$key }}</td>
                                                    <td>{{ $typepersonal->tps_name }}</td>
                                                    <td>
                                                        <a href="{{ url('typeperson',$typepersonal->tps_id).'/edit'}}" class="btn btn-outline-warning btn-warning"
                                                            data-toggle="tooltip" data-placement="bottom" title="Edit"><i
                                                                class="fa fa-edit"></i></a>
                                                        <button class="btn btn-outline-danger btn-danger delete_typeperson" id="delete_typeperson"
                                                            atr="{{ $typepersonal->tps_id }}" data-toggle="tooltip"
...
<script>
    $(function () {
        ...
        $(document).on('click', '.delete_typeperson', function () {
            var id = $(this).attr('atr');
            var token = $('input[name=_token]').val();
            swal({
                title: "Warning",
                text: "Do you want to delete Type of personal ?",
                icon: "warning",
                buttons: true,
                dangerMode: true,
            }).then((willDelete) => {
